...
def run_training():
	...
	if use_pretrained_model:
		...
		model = './ckpt/mfb_cross/best/mfb_baseline_ucf24.model-31000'
	...
	try:
		with sess.as_default():
			...
			while not coord.should_stop():
				...
				mfb = mfb_list[0]
				ffg, fbg, lfg, gt_ffg, gt_fbg, gt_lfg, ffg_loss, fbg_loss, lfg_loss, feat_loss, fg_feat, bg_feat, mt_feat = sess.run([
					...
					mfb.fg_feat, mfb.bg_feat, mfb.mt_feat])
				...
				line = '[%d/%d] ffg_loss=%.8f, fbg_loss=%.8f, lfg_loss=%.8f, feat_loss=%.8f' \
						%(cnt, total_steps, ffg_loss, fbg_loss, lfg_loss, feat_loss)
