...
prefix          = 'mfb_cross' + (str(FLAGS.version) if FLAGS.version != 1 else '') + '_lr=' + str(FLAGS.lr) + '_feat=' + str(FLAGS.feat) + '_cl=' + str(FLAGS.cross_layer) + '_diff_dec=' + str(FLAGS.diff_dec) + '_freeze'
...
evaluation_job  = './jobs/mfb_cross23_val'
...
def tower_loss(name_scope, mfb, clips, img_masks, loss_masks): # masks 16 16 128 128
	...
	rec_loss      = first_fg_loss + first_bg_loss
...
def run_training():
		...
		for gpu_index in range(FLAGS.num_gpus):
			with tf.device('/gpu:%d' % (gpu_index)):
				with tf.name_scope('%s_%d' % ('tower', gpu_index)) as scope:
					...
					lfg_grads = opt.compute_gradients(last_fg_loss, var_list=[v for v in vars_to_optimize if 'fg_dec' not in v.name and 'bg_dec' not in v.name])

					tower_grads.append(grads)
					tower_lfg_grads.append(lfg_grads)
	...
	lfg_grads = average_gradients(tower_lfg_grads)
	with tf.control_dependencies(update_ops):
		train_op = opt.apply_gradients(grads, global_step=global_step)
		train_op_lfg = opt.apply_gradients(lfg_grads, global_step=global_step)
	...
	try:
		with sess.as_default():
			...
			while not coord.should_stop():
				...
				sess.run([train_op, train_op_lfg])
