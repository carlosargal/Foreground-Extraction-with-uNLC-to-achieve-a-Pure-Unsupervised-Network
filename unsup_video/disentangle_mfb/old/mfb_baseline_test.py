...
def run_training():
	...
	masks_placeholder = tf.placeholder(tf.bool, shape=(
											...
											FLAGS.width))"""
	tower_grads  = []
	tower_losses, tower_ffg_losses, tower_fbg_losses, tower_lfg_losses, tower_wd_losses = [], [], [], [], []
		...
		for gpu_index in range(FLAGS.num_gpus):
			with tf.device('/gpu:%d' % (gpu_index)):
				with tf.name_scope('%s_%d' % ('tower', gpu_index)) as scope:
					...
					if test_mode == 0:
						clips = clips * (1-tf.expand_dims(img_masks, -1))
					elif test_mode == 1:
						clips = clips * tf.expand_dims(img_masks, -1)
					elif test_mode == 2:
						frames = tf.expand_dims(clips[:,0], 1)
						clips  = tf.tile(frames, [1,FLAGS.seq_length,1,1,1])

					mfb = mfb_net(clips, FLAGS.height, FLAGS.width, FLAGS.seq_length, FLAGS.channel, FLAGS.batch_size)
					mfb_list.append(mfb)
					loss, first_fg_loss, first_bg_loss, last_fg_loss, wd_loss = tower_loss(scope, mfb, clips, img_masks, loss_masks)
	...
	try:
		with sess.as_default():
			...
			while not coord.should_stop():
				# Run testing
				"""
				fg_feat, bg_feat, mt_feat = 0, 0, 0
				for mfb in mfb_list:
					fg_feat_, bg_feat_, mt_feat_ = sess.run([mfb.fg_feat, mfb.bg_feat, mfb.mt_feat])
					fg_feat += fg_feat_
					bg_feat += bg_feat_
					mt_feat += mt_feat_
				fg_feat /= len(mfb_list)
				bg_feat /= len(mfb_list)
				mt_feat /= len(mfb_list)

				print('test mode = %d: \n'%test_mode)

				np.save('fg_feat', fg_feat)
				np.save('bg_feat', bg_feat)
				np.save('mt_feat', mt_feat)
				"""
