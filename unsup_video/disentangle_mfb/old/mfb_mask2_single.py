...
def tower_loss(name_scope, mfb, clips, img_masks, loss_masks): # masks 16 16 128 128
	...
	s_ffg   = tf.reduce_sum(img_masks[:,0])
	s_lfg   = tf.reduce_sum(img_masks[:,-1])
	w1      = 1 + tf.abs(s_total - 2*s_ffg) / s_ffg
	w2      = 1 + tf.abs(s_total - 2*s_ffg) / (s_total - s_ffg)
	w3      = 1 + tf.abs(s_total - 2*s_lfg) / s_lfg
	...
	first_fg_loss = tf.reduce_mean(tf.abs(first_fg_rec-first_frames)*img_masks[:,0]*w1)
	first_bg_loss = tf.reduce_mean(tf.abs(first_bg_rec-first_frames)*(1-img_masks[:,0])*w2)
	last_fg_loss  = tf.reduce_mean(tf.abs(last_fg_rec-last_frames)*img_masks[:,-1]*w3)
		...
		for gpu_index in range(FLAGS.num_gpus):
			with tf.device('/gpu:%d' % (gpu_index)):
				with tf.name_scope('%s_%d' % ('tower', gpu_index)) as scope:
					...
					mfb_list.append(mfb)
					loss, first_fg_loss, first_bg_loss, last_fg_loss, mffg_loss, mlfg_loss = tower_loss(scope, mfb, clips, img_masks, loss_masks)
	...
	try:
		with sess.as_default():
			...
			while not coord.should_stop():
				...
				if step % 10000000 == 0 and step >= 10000000: # save checkpoint
