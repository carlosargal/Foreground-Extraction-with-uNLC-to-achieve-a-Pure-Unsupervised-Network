...
evaluation_job  = './jobs/mfb_baseline_val'
		...
		for gpu_index in range(FLAGS.num_gpus):
			with tf.device('/gpu:%d' % (gpu_index)):
				with tf.name_scope('%s_%d' % ('tower', gpu_index)) as scope:
					...
					mfb_list.append(mfb)
					loss, first_fg_loss, first_bg_loss, last_fg_loss, wd_loss = tower_loss(scope, mfb, clips_list[gpu_index], img_masks_list[gpu_index], loss_masks_list[gpu_index])
		...
		with sess.as_default():
			...
			while not coord.should_stop():
				...
				if step == 1 or step % 10 == 0: # evaluate loss
					loss, ffg_loss, fbg_loss, lfg_loss, wd_loss, lr = \
						sess.run([loss_op, ffg_loss_op, fbg_loss_op, lfg_loss_op, wd_loss_op, learning_rate])
					line = 'step %d/%d, loss=%.8f, ffg=%.8f, fbg=%.8f, lfg=%.8f, lwd=%.8f, dur=%.3f, lr=%.8f' \
							%(step, total_steps, loss, ffg_loss, fbg_loss, lfg_loss, wd_loss, duration, lr)
					print(line)
