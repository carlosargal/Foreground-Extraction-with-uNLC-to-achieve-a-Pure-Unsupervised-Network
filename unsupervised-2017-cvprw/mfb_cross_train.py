...
flags.DEFINE_integer('num_epochs', 100, 'Number of epochs.') # ~6 min per epoch
...
flags.DEFINE_integer('num_sample', 1800, 'Number of samples in this dataset.') # Depends on the dataset used
flags.DEFINE_float('wd', 0.001, 'Weight decay rate.')

FLAGS = flags.FLAGS

prefix          = 'mfb_cross_uNLC'
...
def run_training():
	...
	model_filename = "./mfb_uNLC_ucf24.model"
		...
		with sess.as_default():
			...
			while not coord.should_stop():
				...
				if step % 100 == 0 and save_predictions: # save current predictions
					...
					imageio.imwrite(os.path.join(pred_save_dir, '%07d.jpg'%step), save_img)
