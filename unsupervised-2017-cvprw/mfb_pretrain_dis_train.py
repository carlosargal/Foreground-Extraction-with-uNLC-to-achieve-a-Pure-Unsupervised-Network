...
flags.DEFINE_integer('num_sample', 1373, 'Number of samples in train_uNLC dataset.')
#flags.DEFINE_integer('num_sample', 1900, 'Number of samples in train_baseline dataset.')
...
loss_save_dir   = './loss'
train_list_path = './dataset/train_record_list.txt'
dataset_path    = './dataset/UCF-101-uNLC-tf-records'
# evaluation_job  = './jobs/mfb_pretrain_dis_val'
encoder_path    = './ckpt/mfb_cross_uNLC/mfb_uNLC_ucf24.model-13500'
	...
	try:
		with sess.as_default():
			...
			while not coord.should_stop():
				...
				if step == 1 or step % 100 == 0: # save checkpoint
					saver.save(sess, os.path.join(model_save_dir, model_filename), global_step=global_step)

				if step == 1 or step % 100 == 0: # validate
					pass
					#from mfb_pretrain_dis_val import run_validating
					#subprocess.call(['srun', '--gres=gpu:1', '--mem 20G', 'python', 'mfb_pretrain_dis_val.py'])
