...
def im2vid(vidPath, imSeq, maskSeq=None):
    ...
    """
    import cv2
    writer = cv2.VideoWriter(vidPath, cv2.VideoWriter_fourcc(*'MJPG'), 10, (imSeq[0].shape[1], imSeq[0].shape[0]))
...
def read_r(indir, pattern):
    ...
    """
    import glob
    flist = sorted([y for (x, _, _) in os.walk(indir) for y in glob.glob(os.path.join(x, pattern))])
    return flist
